...
/* =========================================================
   QUICK CREATE SURVEY-LOCAL QUESTION
========================================================= */

async function quickCreateQuestion({
  question_text,
} = {}) {
  ...
  const preferredQuestionType =
    workspace.question_types.find(
      (questionType) => {
        const searchableValue = [
          questionType.type_code,
          questionType.type_name,
          questionType.rendering_component,
        ]
          .filter(Boolean)
          .join(" ")
          .toLowerCase();
  ...
  if (
    ...
    setErrorMessage(
      "No question type is available for quick creation."
    );
  ...
  try {
    ...
    console.error(
      "Quick question creation failed:",
      error
    );
